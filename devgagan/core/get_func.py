...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    try:
        ...
        if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
            ...
            if chat in saved_channel_ids:
                await app.edit_message_text(
                    ...
                    "Sorry! This channel is protected by **__Team JB__**."
                )
                return
        ...
        # Upload media
        free_check = await chk_user(chat, sender)
...
async def split_and_upload_file(app, sender, target_chat_id, file_path, caption, topic_id):
    """Split large files into parts and upload them"""
    ...
    start_msg = await app.send_message(sender, f"ℹ️ File size: {file_size / (1024 * 1024 * 1024):.2f} GB - Splitting...")
    
    # Calculate number of parts needed (1.9GB per part)
    PART_SIZE = int(1.9 * 1024 * 1024 * 1024)  # 1.9 GB in bytes
    ...
    async with aiofiles.open(file_path, "rb") as f:
        while True:
            ...
            part_file = f"{file_name}.part{str(part_number).zfill(3)}{file_ext}"
            part_created = False
            
            # Write to part file in 64MB chunks
            ...
            # Upload the part
            edit_msg = await app.send_message(
                sender, 
            ...
            try:
                ...
                await app.send_message(sender, f"❌ Part upload failed: {str(e)}")
    ...
    os.remove(file_path)
    await app.send_message(sender, f"✅ Successfully uploaded {part_number} parts!")
...
async def rename_file(file, sender):
    ...
    # Sanitize filename
    sanitized_name = re.sub(r'[\\/*?:"<>|]', "_", original_file_name).strip()
    new_file_name = f"{sanitized_name} {custom_rename_tag}.{file_extension}"
    
    # Atomic rename to prevent corruption
    temp_name = f"temp_{os.urandom(4).hex()}"
...
def progress_callback(done, total, user_id):
    ...
    remaining_time = (total - done) / (speed / elapsed_time) if speed > 0 else 0
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        ...
        await event.edit("Choose your preferred upload method:\n\n__**Note:** **SpyLib ⚡**, built on Telethon(base), by Team JB still in beta.__", buttons=buttons)
